...
final class XmlExplorer {
    ...
    private static List<XmlElement> findTag(String contentXml, XmlTag tag, String open, String close) {
        ...
        while (matcher.find()) {
            ...
            int closingIndex = findClosingIndex(line) + 1;
            XmlElement newTag = new XmlElement(line, tag, closingIndex, new Interval(start, end));
            tags.add(newTag);
        }
    ...
    private static int findSimpleEnding(String contentXml, int start, String open) {
        ...
        int i = open.length();
        i = findClosingIndex(cut.substring(i)) + open.length();
        return cut.charAt(i - 1) == SLASH ? start + i + 1 : -1;
    }

    private static int findClosingIndex(String contentXml) {
        int i = 0;
        char current = contentXml.charAt(i);
        char quote = NUL;
        while (current != '>' || quote != NUL) {
            i++;
